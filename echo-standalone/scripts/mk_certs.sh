...
# to create PKCS12 key stores and trust stores for the two Arrowhead systems
...
## Both systems are given X.509 Subject Alternative Names with both IP-addresses
## and DNS names. This is intended to make these certificates useful for being
## deployed using e.g. Docker.
...
create_cloud_system_keystore "echo_consumer"     "ip:192.168.1.20,dns:echo-consumer.local"
create_cloud_system_keystore "echo_provider"     "ip:192.168.1.21,dns:echo-provider.local"

## The same trust store can be used by both systems, as the only certificate
## in the trust store is that of the cloud they are both members of.
create_truststore \
